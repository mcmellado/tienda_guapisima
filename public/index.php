...
<body>
    ...
    <div class="container mx-auto">
        ...
        <?php if ($usuario->es_admin()): ?> 
            <a href="/admin/index.php" class="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">Administrar tienda</a>
        <?php endif ?>
        </div>
        <br>
            ...
            <main class="flex-1 grid grid-cols-3 gap-4 justify-center justify-items-center">
                <?php foreach ($sent as $fila) : ?>
                    ...
                    <div class="p-6 max-w-xs min-w-full bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <?php if ($fila['stock'] > 0): ?>
                            <a href="/insertar_en_carrito.php?id=<?= $fila[0] ?>&cat=<?= $fila['id_categoria'] ?>&ordenar=<?=hh($ordenar)?>&desde=<?=hh($desde)?>&hasta=<?= hh($hasta)?>" class="inline-flex items-center py-2 px-3.5 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                                Añadir al carrito
